...
// by substring (file names embed the key, e.g. DeepMyCode-darwin-arm64.zip), so the
...
// (deepmycode.key) and the public key (deepmycode.pub) into dir. The password comes
...
func genKey(dir string) error {
	...
	keyPath := filepath.Join(dir, "deepmycode.key")
	pubPath := filepath.Join(dir, "deepmycode.pub")
...
func signFiles(files []string) error {
	...
	for _, f := range files {
		...
		sig := minisign.SignWithComments(priv, data,
			"file:"+filepath.Base(f), "DeepMyCode desktop release")
		out := f + ".minisig"
