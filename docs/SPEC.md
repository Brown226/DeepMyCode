# DeepMyCode Engineering Spec

> DeepMyCode is a coding agent: a thin harness driving multiple models, with **all
...
```
deepmycode/
├── go.mod / go.sum          # module deepmycode; require BurntSushi/toml
├── Makefile                 # build / cross / vet / fmt / test
├── README.md / README.zh-CN.md
├── deepmycode.example.toml         # sample config
├── docs/SPEC.md             # this file
├── cmd/deepmycode/main.go          # entry; blank-imports built-in providers/tools
├── cmd/deepmycode-plugin-example/  # reference MCP stdio plugin (a runnable example)
└── internal/
    ...
    ├── command/             # custom slash commands loaded from .deepmycode/commands/*.md
...
- `cmd/deepmycode-plugin-example` is a runnable reference stdio server (`echo`,
...
Long tasks eventually fill the model's context window. DeepMyCode manages this with
...
- Compaction summarizes the older middle of the session into a single briefing —
  ...
  summarized away.
- The dropped originals are archived to `~/.config/deepmycode/archive/<timestamp>.jsonl`
...
- **Resolving `Ask`.** The interactive front-end (the chat TUI) prompts the user
  ...
  (`deepmycode run`, a sub-agent, anything with no TTY / no approver) cannot prompt, so
...
Out of the box (`mode = "ask"`, no rules) `deepmycode run` behaves exactly as before
(writers resolve `Ask`→allow with no TTY), while `deepmycode chat` now prompts before
...
- **Custom commands** are Markdown files under `.deepmycode/commands/` (project) and
  `~/.config/deepmycode/commands/` (user); the project dir overrides the user dir on a
...
- Frontmatter is an optional `---`-fenced block of simple `key: value` lines;
  `description` and `argument-hint` are recognised (no YAML dependency — DeepMyCode
...
Resolution order: **flag > project `./deepmycode.toml` > user `~/.config/deepmycode/config.toml`
...
# language    = "zh"                # ui language tag; empty = auto-detect from $LANG / $DEEPMYCODE_LANG

[agent]
system_prompt = "You are DeepMyCode, a coding agent..."  # or system_prompt_file = "..."
max_steps     = 25
temperature   = 0.0
...
`deepmycode setup` writes this default config so the CLI is usable out of the box.
...
`[[plugins]]`; on a name collision `deepmycode.toml` wins (it is the more explicit,
DeepMyCode-specific source). This lets a server already configured for Claude work in
DeepMyCode unchanged.
...
- Build: `CGO_ENABLED=0 go build -ldflags "-s -w -X main.version=$(VERSION)" -o deepmycode ./cmd/deepmycode`
...
- Sandbox Phase 1: an OS-level jail for `bash` so commands — not just the
  ...
  command unconfined via the permission gate (in `deepmycode run`, the command just
...
- "Always allow" persistence writing learned rules back to project config; a
  per-session permission override flag for `deepmycode run`.
