...
func resetTree(repo string) {
	...
	_ = exec.Command("git", "-C", repo, "clean", "-fd", "-e", "deepmycode.toml").Run()
}
...
func renderDiff(r diffReport) string {
	...
	fmt.Fprintf(&b, "## 🤖 DeepMyCode e2e — diff test-gen\n\n")
...
// repoRelFromModulePath turns "deepmycode/internal/agent/foo.go" into
