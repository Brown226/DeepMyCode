...
All notable changes to the Go line (DeepMyCode 1.0+) are recorded here. The legacy
`0.x` TypeScript history lives on the [`v1`](https://github.com/Brown226/DeepMyCode/tree/v1)
branch.
...
- **Go kernel**: a single static binary (CGO-free), cross-compiled for
  ...
  the native binary), Homebrew (`Brown226/DeepMyCode` tap), and release archives;
...
- **Code intelligence via CodeGraph**: a tree-sitter symbol/call graph
  ...
  or API cost. Fetched into a local cache on first use (or `deepmycode codegraph
...
- **Memory**: `DEEPMYCODE.md` hierarchy + auto-memory, folded into the cache-stable
  prefix.
- **ACP** (`deepmycode acp`) and an HTTP/SSE server frontend; desktop app (Wails).
...
[1.0.0]: https://github.com/Brown226/DeepMyCode/releases/tag/v1.0.0
