...
build:
	CGO_ENABLED=0 go build -ldflags "$(LDFLAGS)" -o bin/deepmycode$(GOEXE) ./cmd/reasonix
	CGO_ENABLED=0 go build -ldflags "$(LDFLAGS)" -o bin/deepmycode-plugin-example$(GOEXE) ./cmd/reasonix-plugin-example
...
cross:
	...
	@for p in darwin/amd64 darwin/arm64 linux/amd64 linux/arm64 windows/amd64 windows/arm64; do \
		...
		echo "build $$os/$$arch"; \
		CGO_ENABLED=0 GOOS=$$os GOARCH=$$arch go build -ldflags "$(LDFLAGS)" -o dist/deepmycode-$$os-$$arch$$ext ./cmd/reasonix; \
	done
...
# layout: launcher at bin/codegraph/bin/codegraph beside bin/deepmycode) and run the
# gated MCP end-to-end test against it. Requires `gh`. Windows: install via the
# upstream install.ps1 and run the test with DEEPMYCODE_CODEGRAPH_BIN set.
e2e-codegraph:
	...
	DEEPMYCODE_CODEGRAPH_E2E=1 DEEPMYCODE_CODEGRAPH_BIN=$$PWD/$$dest/bin/codegraph \
