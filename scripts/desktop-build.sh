...
#   macOS:   DeepMyCode-darwin-<arch>.zip                  (ditto archive of the .app)
#   Windows: DeepMyCode-windows-<arch>-installer.exe       (NSIS per-user installer)
#   Linux:   DeepMyCode-linux-<arch>.tar.gz                (bare binary)
#
...
APPNAME="DeepMyCode"            # wails.json productName -> DeepMyCode.app
BINNAME="deepmycode-desktop"    # wails.json outputfilename -> linux binary name
...
case "$os" in
darwin)
	# Wails names the bundle after outputfilename (deepmycode-desktop.app); repackage
	# it as DeepMyCode.app for a clean user-facing name. Ad-hoc sign the copy (still
	...
	cp -R "build/bin/deepmycode-desktop.app" "$app"
