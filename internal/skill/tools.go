...
func (t *installSkillTool) Description() string {
	scope := "'global' (only option — no project workspace) writes to ~/.deepmycode/skills/."
	if t.store.HasProjectScope() {
		scope = "'project' (default) writes to <repo>/.deepmycode/skills/ (this workspace only); 'global' writes to ~/.deepmycode/skills/ (every project)."
	}
