...
// $LC_ALL / $DEEPMYCODE_LANG. Unknown locales return "" so DetectLanguage falls
...
// TestDetectLanguagePriority verifies override beats env and that DEEPMYCODE_LANG
// beats LANG. With a clean env we fall back to English.
func TestDetectLanguagePriority(t *testing.T) {
	t.Setenv("DEEPMYCODE_LANG", "")
	t.Setenv("LC_ALL", "")
	...
	t.Setenv("DEEPMYCODE_LANG", "en")
	if got := DetectLanguage(""); got != "en" {
		t.Errorf("DEEPMYCODE_LANG=en overriding LANG=zh: got %q, want en", got)
	}
