// Package cli implements DeepMyCode's command-line entry: subcommand routing, flag
...
func Run(args []string, version string) int {
	...
	case "init":
		...
		// for config), so `deepmycode init` isn't a dead end.
	...
	case "version", "--version", "-v":
		fmt.Println("deepmycode", version)
		return 0
	case "help", "--help", "-h":
		usage()
		return 0
...
func runServe(args []string) int {
	...
	fmt.Printf("deepmycode serve — %s on http://%s\n", ctrl.Label(), *addr)
...
// Keys always go to the deepmycode-owned global credentials file so they never land
...
// project-local deepmycode.toml only when the user config dir can't be resolved.
func defaultConfigTarget() string {
	if p := config.UserConfigPath(); p != "" {
		return p
	}
	return "deepmycode.toml"
}

// defaultEnvTarget is the deepmycode-owned global credentials file, falling back to
...
// resolveSetupTargets picks where `deepmycode setup` writes. Keys always go to the
// global env. The config goes to the user-global dir by default, to ./deepmycode.toml
...
// setupConfig runs the configuration wizard (the `deepmycode setup` command),
// writing config.toml to the user-global dir (or ./deepmycode.toml under --local)
// and API keys to the deepmycode-owned global .env — never a project's own .env.
...
// initHint handles `deepmycode init`. Unlike a config scaffold, project memory is
...
// (and to `deepmycode setup` for config) so the verb isn't a dead end.
func initHint() int {
...
// and any entered API keys to envPath (the deepmycode-owned global .env, never a
...
func interactiveSetup(configPath, envPath string) int {
	...
	fmt.Print(boxed([]string{
		accent("◆") + " " + fmt.Sprintf(i18n.M.WelcomeTitleFmt, bold("deepmycode")),
		"",
...
// that older versions of the wizard wrote into deepmycode.toml. They collide
...
// can warn the user to clean up deepmycode.toml by hand.
...
// duplicate provider entries on subsequent re-runs of `deepmycode setup`.
// The host-based slug also gives users a meaningful name to grep for in
// deepmycode.toml. Falls back to a short sha1 of the raw URL when the URL
...
// not exist in config because users editing deepmycode.toml deal with SKU names
...
// deepmycode.toml that defines only [[providers]] for deepseek otherwise hides
...
// deepmycode-owned global .env so the chat session that follows picks them up via
...
// re-run of `deepmycode setup` re-pins the current value into .env (a
...
// are appended — so re-running `deepmycode setup` with a corrected key replaces the
...
func welcome(version string) int {
	...
	// directory without a local deepmycode.toml (SourcePath is then "").
	...
	b.WriteString(boxed([]string{
		accent("◆") + " " + bold("deepmycode") + "  " + dim(version),
		dim(i18n.M.Subtitle),
	}))
	...
	if src == "" {
		step("deepmycode setup", i18n.M.StepScaffold)
	}
	if ready == 0 {
		step(i18n.M.StepSetKey, i18n.M.StepSetKeyHint)
	}
	step("deepmycode chat", i18n.M.StepChatDesc)
	step(`deepmycode run "task"`, i18n.M.StepRunDesc)
