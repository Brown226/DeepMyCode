...
func configureCLIThemeWithStyle(mode, style string) {
	if env := strings.TrimSpace(os.Getenv("DEEPMYCODE_THEME")); env != "" {
	...
	} else if env := strings.TrimSpace(os.Getenv("REASONIX_THEME")); env != "" {
		if st, ok := cliThemeStyleByName(env); ok {
			mode = st.mode
			style = st.name
		} else {
	...
	if env := strings.TrimSpace(os.Getenv("DEEPMYCODE_THEME_STYLE")); env != "" {
		style = env
	} else if env := strings.TrimSpace(os.Getenv("REASONIX_THEME_STYLE")); env != "" {
		style = env
	}
