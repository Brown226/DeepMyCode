...
// <user cache>/deepmycode/codegraph/<Version>. Versioned so a bump installs cleanly
// beside the old one. DEEPMYCODE_CACHE_DIR overrides the base (relocate the cache,
// or isolate it in tests); REASONIX_CACHE_DIR is the legacy fallback. Empty when no
// cache/config dir resolves.
func CacheDir() string {
	base := os.Getenv("DEEPMYCODE_CACHE_DIR")
	if base == "" {
	...
	if base == "" {
		...
		base = filepath.Join(base, "deepmycode")
	}
...
// InstallWithClient is Install with an explicit HTTP client, used when DeepMyCode
...
func InstallWithClient(ctx context.Context, client *http.Client, log func(string)) (string, error) {
	...
	if err := promote(root, dir); err != nil {
		...
		return "", fmt.Errorf("codegraph: install to %s failed: %w — the cache directory may be read-only or locked by antivirus; set DEEPMYCODE_CACHE_DIR to a writable location to relocate it", dir, err)
	}
	p, ok := cached()
	if !ok {
