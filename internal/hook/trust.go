...
// Trust gates project hooks. A project's .deepmycode/settings.json can run
...
// root. The trust flag lives in user-global state (~/.deepmycode/trust.json),
// NOT in the project file itself — an attacker controls the latter. Global
// hooks (~/.deepmycode/settings.json) are the user's own and always run.

// TrustFilename is the user-global trust store under ~/.deepmycode.
...
// TrustPath is ~/.deepmycode/trust.json (homeDir overrides ~).
