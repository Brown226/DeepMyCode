...
// ExtractHostChecks reads only the structured "DeepMyCode host checks" section.
	...
	for _, doc := range docs {
		...
		for i, raw := range strings.Split(doc.Body, "\n") {
			...
			if heading, ok := markdownHeading(line); ok {
				inSection = strings.EqualFold(heading, "DeepMyCode host checks") || strings.EqualFold(heading, "Reasonix host checks")
				continue
			}
			if !inSection {
				continue
